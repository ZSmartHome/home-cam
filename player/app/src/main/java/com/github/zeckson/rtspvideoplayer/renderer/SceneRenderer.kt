...
import com.github.zeckson.rtspvideoplayer.VideoUiView
import com.github.zeckson.rtspvideoplayer.renderer.Utils.checkGlError
...
/* package */ internal constructor(
    ...
    private var externalFrameListener: SurfaceTexture.OnFrameAvailableListener? = null
    ...
    @AnyThread
    @Synchronized
    fun setVideoFrameListener(videoFrameListener: SurfaceTexture.OnFrameAvailableListener) {
        externalFrameListener = videoFrameListener
    }

    companion object {
        private val TAG = "SceneRenderer"
        ...
        fun createFor2D(): SceneRenderer {
            return SceneRenderer(null, null, null, null)
        }
         ...
         */
        @MainThread
        fun createForVR(context: Context, parent: ViewGroup): Pair<SceneRenderer, VideoUiView> {
            val canvasQuad = CanvasQuad()
            val videoUiView = VideoUiView.createForOpenGl(context, parent, canvasQuad)
            val externalFrameListener = videoUiView.frameListener

            val scene = SceneRenderer(
                canvasQuad, videoUiView, Handler(Looper.getMainLooper()), externalFrameListener
            )
            return Pair.create<SceneRenderer, VideoUiView>(scene, videoUiView)
        }
    }
}
